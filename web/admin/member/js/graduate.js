var graduateList;
var name, gender, telephone, company, post, intro, photo;
var uploadInst;
...
function fillStudents() {
    graduateList = arguments[0];
    var tbody = document.getElementsByTagName('tbody')[0];
    $('tbody').html("");

    for (var i = 0; i < graduateList.length; ++i){
        var student = graduateList[i];
        ...
        td4.innerText = student.telephone;
        td4.style.wordWrap = "break-word";
        tr.appendChild(td4);

        var td5 = document.createElement('td');
        td5.style.textAlign = "center";
        td5.innerText = student.company;
        td5.style.wordWrap = "break-word";
        tr.appendChild(td5);

        var td6 = document.createElement('td');
        td6.style.textAlign = "center";
        td6.innerText = student.post;
...
function openAddStudent() {
    ...
    layer.open({
        ...
        content:
            ...
            '        <div class="layui-input-block">\n' +
            '            <input class="layui-input" id="student_post" type="text" name="post" placeholder="请输入岗位" autocomplete="off" style="width: 250px;" onchange="inputLimit()" onkeydown="inputLimit()" onkeyup="inputLimit()">\n' +
            '        </div>\n' +
            '    </div>\n' +
            '    <div class="layui-form-item">\n' +
            ...
            '                addstudent(name, gender, photo, telephone, company, post, intro);\n' +
            '            }\n' +
            '\n' +
            '        }\n' +
            '        , error: function () {\n' +
...
function addStudent() {
    ...
    company = $('#student_company').val();
    post = $('#student_post').val();
    if(photo == "" || photo == null){
        layer.msg("请上传照片！");
        return false;
    }
...
function addstudent(name, gender, photo, telephone, company, post, intro) {
    $.ajax({
        url : 'addStudent.action',
        type : 'post',
        data : {"name" : name, "photo" : photo, "gender" : gender, "telephone" : telephone, "company" : company, "post" : post, "intro" : intro, "type" : 3},
        scriptCharset : 'utf-8',
        success : function (result) {
            if(result.message == null) {
                layer.open({
...
function editStudent() {
    ...
    var student = graduateList[num-1];
    if(student.gender == "男"){
        layer.open({
            type: 1,
            offset: 'auto',
            ...
            content:
                ...
                '        <label class="layui-form-label">公司/学校</label>\n' +
                '        <div class="layui-input-block">\n' +
                '            <input type="text" id="student_company"  autocomplete="off" placeholder="请输入公司/学校" class="layui-input" style="width: 250px;" onchange="inputLimit()" onkeydown="inputLimit()" onkeyup="inputLimit()" value="'+student.company+'">\n' +
                '        </div>\n' +
                '    </div>\n' +
                '    <div class="layui-form-item">\n' +
                '        <label class="layui-form-label">岗位</label>\n' +
                '        <div class="layui-input-block">\n' +
                '            <input type="text" id="student_post"  autocomplete="off" placeholder="请输入岗位" class="layui-input" style="width: 250px;" onchange="inputLimit()" onkeydown="inputLimit()" onkeyup="inputLimit()" value="'+student.post+'">\n' +
                '        </div>\n' +
                '    </div>\n' +
                '    <div class="layui-form-item">\n' +
                '        <label class="layui-form-label">简介</label>\n' +
                ...
                '                photo = res.src;\n' +
                '                $(\'#demo1\').attr(\'src\', photo);\n' +
                '                updateStudent(name, gender, photo, telephone, company, post, intro);\n' +
                '}\n' +
                '\n' +
                '        }\n' +
                '        , error: function () {\n' +
        ...
        layer.open({
            ...
            content:
                ...
                '        <label class="layui-form-label">公司/学校</label>\n' +
                '        <div class="layui-input-block">\n' +
                '            <input type="text" id="student_company"  autocomplete="off" placeholder="请输入公司" class="layui-input" style="width: 250px;" onchange="inputLimit()" onkeydown="inputLimit()" onkeyup="inputLimit()" value="'+student.company+'">\n' +
                '        </div>\n' +
                '    </div>\n' +
                '    <div class="layui-form-item">\n' +
                '        <label class="layui-form-label">岗位</label>\n' +
...
function editstudent() {
    ...
    if(name == "" || name == null){
        ...
        updateStudent(name, gender, student.photo, telephone, company, post, intro);
    }
...
function updateStudent(name, gender, photo, telephone, company, post, intro) {
    $.ajax({
...
function delStudent() {
    ...
    layer.open({
        ...
        offset: 'auto', //具体配置参考：http://www.layui.com/doc/modules/layer.html#offset
        id: 'layerDemo1', //防止重复弹出
        content: '<div style="padding: 20px 50px;">' + "确定删除毕业生 <b>'" + student.name + "'</b> 吗？" + '</div>',
        btn: ['确定', '取消'],
        btnAlign: 'c', //按钮居中
        shade: 0.5, //不显示遮罩
        title: "删除毕业生",
