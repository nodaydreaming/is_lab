    ...
    if(instructor.gender == "男"){
        layer.open({
            ...
            content:
                ...
                '        <label class="layui-form-label">邮箱</label>\n' +
                '        <div class="layui-input-block">\n' +
                '            <input class="layui-input" id="instructor_email" type="text" name="email" placeholder="请输入邮箱" autocomplete="off" style="width: 250px;" value="'+instructor.email+'">\n' +
                '        </div>\n' +
                '    </div>\n' +
                '    <div class="layui-form-item">\n' +
                '        <label class="layui-form-label">简介</label>\n' +
...
function editinstructor() {
    ...
    if(photo == "" || photo == null){
        ...
        if (photo != instructor.photo) {
            uploadInst.upload();
        }
        updateInstructor(instructor.teacherId, name, gender, photo, email, degree, intro);
    }
}

function updateInstructor(id, name, gender, photo, email, degree, intro) {
    console.log({"teacherId" : id, "name" : name, "photo" : photo, "gender" : gender, "email" : email, "degree" : degree, "intro" : intro});
    $.ajax({
        url : 'updateInstructor.action',
        type : 'post',
        data : {"teacherId" : id, "name" : name, "photo" : photo, "gender" : gender, "email" : email, "degree" : degree, "intro" : intro},
        scriptCharset : 'utf-8',
        success : function (result) {
            if(result.message == null) {
                layer.open({
